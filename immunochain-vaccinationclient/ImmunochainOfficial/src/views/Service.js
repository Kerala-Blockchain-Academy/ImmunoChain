...
const Male_Female_count_URL = Client_config.male_female_count_API
const Beneficiary_report_URL = Client_config.beneficiary_report_API
...
export function date_of_birth(date) {
  if (date != null) {

    let value = pad(date.dd, 2) + "-" + pad(date.mm, 2) + "-" + date.yyyy;
    let age = moment(new Date(date.yyyy, date.mm - 1, date.dd + 1))
    let today = moment(new Date())
    let numofDays = Math.trunc(moment.duration(today.diff(age)).asDays());
    let result = moment.preciseDiff(age, today, true)

    console.log("---number of Days----", numofDays)

    var difference = ""

    if (!result.firstDateWasLater) {

      if (numofDays < 76) {
        difference = difference + numofDays + " Days "
      }

      else {

        if (result.years) {
          difference = difference + result.years + " Years "
        }
        if (result.months) {
          difference = difference + result.months + " Months "
        }
        if (result.days) {
          difference = difference + result.days + " Days "
        }
...
export async function _get_station_details() {
  try {
    ...
    console.log("statiiiii", station_data)
    return station_data
  } catch (error) {
    console.log(error)
  }
...
export async function _get_report(data) {
  try {
    let response = _Post_Api_(Male_Female_count_URL, data)
    return response
  }
  catch (error) {
    console.error(error);
...
export async function _get_beneficiary_report(data) {
  try {
    let response = _Post_Api_(Beneficiary_report_URL, data)
    return response
  }
  catch (error) {
    console.error(error);
...
export async function asyncClear() {
  ...
  let filtered = ["REFRESH_KEY", "STORAGE_KEY"]
  console.log("---Filtered---", filtered)
  if (filtered.length) {
    AsyncStorage.multiRemove(filtered, () => {
